...
import 'package:curdapp/keys.dart';
...
class RegisterScreen extends StatelessWidget {
  RegisterScreen() : super(key: CurdappKeys.registerScreen);
  ...
  Widget header(BuildContext context) {
    return Column(
      children: <Widget>[
        Image(
          image:
              AssetImage('assets/images/jhipster_family_member_0_head-256.png'),
          fit: BoxFit.fill,
    ...
    return BlocBuilder<RegisterBloc, RegisterState>(
        ...
        builder: (context, state) {
          return TextFormField(
              ...
              onChanged: (value) {
                context.read<RegisterBloc>().add(LoginChanged(login: value));
              },
          ...
          return TextFormField(
              onChanged: (value) {
                context.read<RegisterBloc>().add(ConfirmPasswordChanged(
                    confirmPassword: value, password: state.password.value));
              },
              obscureText: true,
              decoration: InputDecoration(
                  labelText: 'Confirm password',
                  errorText: state.confirmPassword.invalid
                      ? ConfirmPasswordValidationError.invalid.invalidMessage
                      : null));
        });
  }
        ...
        builder: (context, state) {
          return Column(
            children: <Widget>[
              ...
              Visibility(
                visible:
                    state.termsAndConditions.status != FormzInputStatus.pure &&
                        state.termsAndConditions.invalid,
                child: Padding(
          ...
          return Visibility(
              visible: state.status.isSubmissionFailure ||
                  state.status.isInvalid &&
                      state.generalErrorKey != HttpUtils.generalNoErrorKey,
              child: Center(
                child: Text(
  ...
  String generateError(RegisterState state, BuildContext context) {
    ...
    if (state.generalErrorKey
            .toString()
            .compareTo(RegisterBloc.passwordNotIdenticalKey) ==
        0) {
      errorTranslated = 'The passwords are not identical';
    } else if (state.generalErrorKey
            .toString()
            .compareTo(RegisterBloc.emailExistKey) ==
        0) {
      errorTranslated = 'Email already exist';
    } else if (state.generalErrorKey
            .toString()
            .compareTo(RegisterBloc.loginExistKey) ==
        0) {
            ...
            child: Center(
              child: Column(
                ...
                children: <Widget>[
                  ...
                  ElevatedButton(
                    child: Container(child: Center(child: Text('Login'))),
                    onPressed: () =>
